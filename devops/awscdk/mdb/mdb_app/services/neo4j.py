...
class neo4jService:
  def createService(self, config):
    ...
    environment={
        ...
        "NEO4J_dbms_security_procedures_unrestricted":config['db']['apoc_sec_proc'],
        "NEO4J_dbms_directories_plugins":config['db']['neo4j_plugin'],
    }
    ...
    subnet_db1 = config.get('Subnets', 'subnet_db1')
    subnet_db2 = config.get('Subnets', 'subnet_db2')
    ...
    self.NLB = elbv2.NetworkLoadBalancer(self,
        ...
        vpc_subnets=ec2.SubnetSelection(
            subnets=self.VPC.select_subnets(one_per_az=True, subnet_type=ec2.SubnetType.PUBLIC_WITH_EGRESS).subnets
        )
    )

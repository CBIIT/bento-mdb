...
class stsfastapiService:
  def createService(self, config):

    ### STS FAST API Service ###############################################################################################################
    service = "stsfastapi"
    ...
    environment={
        "NEO4J_MDB_URI":f"bolt://{self.NLB.load_balancer_dns_name}:{bolt_port}",
#        "NEO4J_MDB_URI":"bolt://{}:{}".format(self.NLB.load_balancer_dns_name, config.getint('neo4j', 'bolt_port)),
        "NEO4J_MDB_USER":config['db']['neo4j_user'],
...
#    }
    ...
    taskDefinition = ecs.FargateTaskDefinition(self,
        ...
        family=f"{config['main']['resource_prefix']}-{config['main']['tier']}-sts-fast-api",
    ...
    subnet_ecs_api1 = config.get('Subnets', 'subnet_ecs_api1')
    subnet_ecs_api2 = config.get('Subnets', 'subnet_ecs_api2')
    subnets_ecs_api = ec2.SubnetSelection(
        subnets=[
            ec2.Subnet.from_subnet_id(self, "Subnet_ecs_api1", subnet_ecs_api1),
            ec2.Subnet.from_subnet_id(self, "Subnet_ecs_api2", subnet_ecs_api2)
        ]
    )

    ecsService = ecs.FargateService(self,
        "{}-{}-service".format(self.namingPrefix, service),
        service_name=f"{config['main']['resource_prefix']}-{config['main']['tier']}-sts-fast-api",
    ...
    ecsTarget = self.listener.add_targets("ECS-{}-Target".format(service),
        ...
        target_group_name=f"{config['main']['resource_prefix']}-{config['main']['tier']}-sts-fast-api",

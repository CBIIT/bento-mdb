...
def get_cdes_from_mdb(mdb: MDB) -> list[MDBCDESpec]:
    ...
    qry = (
        ...
        "cde.value AS CDEFullName, cde.origin_name AS CDEOrigin, cde.origin_id + '|' + COALESCE(cde.origin_version, '') AS cde_hdl "
        "OPTIONAL MATCH (vs:value_set {handle: cde_hdl})-[:has_term]->(pv:term) "
        "WITH CDECode, CDEVersion, CDEFullName, CDEOrigin, COLLECT(DISTINCT pv) AS pvs "
        ...
        "WITH CDECode, CDEVersion, CDEFullName, CDEOrigin, pv, COLLECT(DISTINCT {value: syn.value, origin_id: "
        "syn.origin_id, origin_name: syn.origin_name, origin_version: "
        "syn.origin_version}) AS synonyms "
        "OPTIONAL MATCH (pv)-[:represents]->(c_alt:concept)<-[:represents]-"
        "(alt_pv:term {origin_name: 'caDSR_alternates'}), (c_alt)-[:has_tag]->(:tag {key: 'mapping_source', value: 'alternate_name'}) "
        "WHERE pv <> alt_pv AND alt_pv.value IS NOT NULL "
        "WITH CDECode, CDEVersion, CDEFullName, CDEOrigin, pv, synonyms,"
        "collect(DISTINCT CASE WHEN alt_pv.value IS NOT NULL THEN {value: alt_pv.value} END) AS alt_tmp "
        "WITH CDECode, CDEVersion, CDEFullName, CDEOrigin, pv, synonyms, [x IN alt_tmp WHERE x IS NOT NULL] AS alternate_values "
        "WITH CDECode, CDEVersion, CDEFullName, CDEOrigin, CASE WHEN pv IS NOT NULL THEN COLLECT(DISTINCT {value: pv.value, origin_id: pv.origin_id, "
        "origin_definition: pv.origin_definition, origin_version: pv.origin_version, "
        "origin_name: pv.origin_name, synonyms: synonyms, alternates: alternate_values}) "
        "ELSE [] END AS permissibleValues "
        "RETURN CDECode, CDEVersion, CDEFullName, CDEOrigin, permissibleValues "
    )

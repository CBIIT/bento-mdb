...
def get_cdes_from_mdb(mdb: MDB) -> list[MDBCDESpec]:
    ...
    qry = (
        ...
        "WITH cde, models, pv, synonyms,"
        "collect(DISTINCT CASE WHEN alt_pv.value IS NOT NULL THEN {value: alt_pv.value} END) AS alt_tmp "
        "WITH cde, models, pv, synonyms, [x IN alt_tmp WHERE x IS NOT NULL] AS alternate_values "
        "WITH cde, models, CASE WHEN pv IS NOT NULL THEN COLLECT(DISTINCT {value: pv.value, origin_id: pv.origin_id, "
